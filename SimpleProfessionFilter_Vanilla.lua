...
-- Check if query matches as a word ending (ends at word boundary: end of string, space, or punctuation)
local function IsWordEndingMatch(source, query)
    if not source or not query or query == "" then return false end
    local start = 1
    while true do
        local s, e = string_find(source, query, start, true)
        if not s then return false end
        -- Check boundary AFTER query (end of string, space, or punctuation)
        local nextChar = source:sub(e + 1, e + 1)
        if nextChar == "" or nextChar == " " or nextChar == "-" or nextChar == ":" or nextChar == "," or nextChar == "." or nextChar == "(" or nextChar == ")" or nextChar == "[" or nextChar == "]" or nextChar == "/" then
            return true
        end
        start = s + 1
    end
...
local TWO_HAND_PATTERNS = {
    "2h", "2-h", "2 h", "two-hand", "two hand", "2-hand", "2 hand",
    "zweihand", "2h-waffe", "zweihandwaffe",
    "2m", "deux mains", "deux-mains", "2 mains",
    "dos manos", "dos-manos",
    "duas mãos", "duas maos",
    "양손", "雙手", "双手"
}

local function IsTwoHandWeapon(nameLower)
    for _, pattern in ipairs(TWO_HAND_PATTERNS) do
        if string_find(nameLower, pattern, 1, true) then
            return true
        end
    end
    return false
...
local function MatchCraftCategory(nameLower, catKey)
    if not nameLower or nameLower == "" then return false end

    if catKey == "All" then
        return true
    end

    if catKey == "TwoHWeapon" then
        if IsTwoHandWeapon(nameLower) then
            return true
        end
        local localizedMatch = L["TwoHWeapon"]
        if localizedMatch and IsWordEndingMatch(nameLower, string_lower(localizedMatch)) then
            return true
        end
        return false
    end
    ...
    if catKey == "Weapon" then
        -- 1-Hand Weapon excludes Two-Handed weapons
        if IsTwoHandWeapon(nameLower) then
            return false
        end
        local localizedMatch = L["Weapon"]
        if localizedMatch and IsWordEndingMatch(nameLower, string_lower(localizedMatch)) then
            return true
        end
        return false
    ...
    if catKey == "Other" then
        for _, key in ipairs(CRAFT_CATEGORIES) do
            if key ~= "All" and key ~= "Other" then
                if MatchCraftCategory(nameLower, key) then
                    return false
                end
            end
        end
    ...
    -- Check synonyms if defined for this category
    if SPF.CategorySynonyms and SPF.CategorySynonyms[catKey] then
        for _, synonym in ipairs(SPF.CategorySynonyms[catKey]) do
            if IsWordEndingMatch(nameLower, synonym) then
                return true
            end
        end
    end
...
function SPF:TryInsertLink(text)
    ...
    -- 3. If NO box is focused, check for redirection to SPF search box
    local shouldInsert = false
    if isShiftAltClick then
        shouldInsert = SimpleProfessionFilterDB.insertWithoutFocusAlt
    else
        shouldInsert = SimpleProfessionFilterDB.insertWithoutFocus
    end
    ...
    -- 4. Grace period fallback (if user just clicked away)
...
-- ============================================================================
-- Auctionator Price Universal Hook
-- ============================================================================
local function AppendAuctionatorPrice(frameType)
    if not Auctionator or not Auctionator.API or not Auctionator.API.v1 or not Auctionator.API.v1.GetAuctionPriceByItemLink then return end
    
    local isCraft = (frameType == "Craft")
    local numButtons = isCraft and CRAFTS_DISPLAYED or TRADE_SKILLS_DISPLAYED
    if not numButtons then return end
    
    local buttonPrefix = isCraft and "Craft" or "TradeSkillSkill"
    local getInfo = isCraft and GetCraftInfo or GetTradeSkillInfo
    local getLink = isCraft and GetCraftItemLink or GetTradeSkillItemLink
    
    if not getLink then return end
    ...
    for i = 1, numButtons do
        ...
        if button and button:IsShown() then
            ...
            if index and index > 0 then
                local name, info2, typeOrCraftType = getInfo(index)
                local typeStr = isCraft and typeOrCraftType or info2
                
                if typeStr ~= "header" then
                    local link = getLink(index)
                    if link and _G.type(link) == "string" and link:find("Hitem:") then
                        local price = Auctionator.API.v1.GetAuctionPriceByItemLink("SimpleProfessionFilter", link)
                        if price and price > 0 then
                            local priceText = "  " .. GetCoinTextureString(price)
                            local currentText = button:GetText() or ""
                            if not currentText:find(priceText, 1, true) then
                                button:SetText(currentText .. priceText)
                            end
                        end
                    end
                end
